...
class ReportBuilder:
    ...
    def _writeup_report(
        ...
        markdown = _markdown_writeup(title, sections, exploit_script=exploit_script, solve_script=solve_script)
        report = {
            "kind": "ctf_writeup",
            "title": title,
            "challenge_id": challenge_id,
        ...
        if solve_script:
            report["solve_script"] = solve_script
        return report
...
# Generated by ForgeFlag from RSA recovery evidence.
METHOD = "{method}"
n = {values["n"]}
e = {values["e"]}
c = {values["c"]}
...
def _markdown_writeup(
    ...
    solve_script: dict[str, str] | None = None,
) -> str:
    lines = [f"# {title}", ""]
    ...
    if solve_script:
        language = solve_script.get("language") or "text"
        filename = solve_script.get("filename") or "solve.py"
