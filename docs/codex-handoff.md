...
- Crypto/RSA triage:
  ...
  - extracts common RSA parameters (`n`, `e`, `c`, `p`, `q`, `d`, `phi`) plus numbered common-modulus fields such as `e1/e2/c1/c2`, and PEM key markers
  - `CryptoSolver` records RSA hints, recovers known-factor, low-exponent exact-root, and common-modulus RSA flags, preserves replay parameters, and emits a reproducible `solve_<challenge>.py` script in the Write-up
...
- Web-run hard/expert CTF corpus benchmark:
  ...
  - Covers hidden Web APIs, source route/sink triage, crypto primitive misuse, RSA low-exponent and common-modulus recovery, DNS exfil, TCP stream follow-up, HTTP object export, SMTP stream summaries, mail/PowerShell forensics, packed reverse, format-string pwn, ret2win pwn, pickle sandbox, and Web-to-Java chains
  - Current strict result: 16/16 full score through the Web API
