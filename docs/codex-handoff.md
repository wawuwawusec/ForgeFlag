...
- Local Web UI:
  ...
  - per-run LLM provider/model/API key controls, browser-local config saving including API key, and `/api/llm/test`
  - entered LLM keys can be restored from browser local storage for run/test requests and are never stored in SQLite
