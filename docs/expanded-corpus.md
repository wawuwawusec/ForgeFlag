...
The current expanded corpus has 70 cases:
...
| Crypto | 10 | hex, Base32, Base64, binary ASCII, ROT13, Caesar, Morse, decimal ASCII, octal ASCII, RSA known factors |
...
Result: 70/70 cases reached full score through the Web API.

The run also verified the Web UI state after upload: the local page at `http://127.0.0.1:8080/` showed 93 total challenges and 70 solved challenges after the expanded benchmark run.
